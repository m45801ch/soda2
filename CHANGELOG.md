...
- Gemini Live 即時文字廣播到所有視窗：錄音時藥丸即時顯示辨識文字（最多 20 字），主視窗維持完整顯示
- Gemini Live 支援自訂模型名稱（不再寫死，日後模型更新可直接改名繼續用）
- 雲端 ASR 各服務商各自記錄 API Key（切換服務商自動換 key，不用重貼）

### 修正

- AI 模型列表：API Key 去除前後空白後再抓取；切換廠商時先清空舊列表，避免顯示別家的模型
- 雲端 ASR 切換服務商時自動清空模型欄，避免把舊模型名稱送給新服務商（如 Gemini 模型名送到 GROQ）
- Gemini Live 停止後要等 15 秒才出字：停止時補送 audioStreamEnd，並以 generationComplete 結束等待
- Gemini Live 辨識文字不會貼到游標：結果改走完成回呼貼上
- Gemini Live 連線錯誤顯示關閉碼與原因，方便排查（如 key 無效）
